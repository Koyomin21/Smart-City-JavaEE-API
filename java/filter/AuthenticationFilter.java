...
public class AuthenticationFilter implements ContainerRequestFilter {
    ...
    public void filter(ContainerRequestContext requestContext) {
        ...
        if(!method.isAnnotationPresent(PermitAll.class) && !method.isAnnotationPresent(JWTTokenNeeded.class)) {
            System.out.println("NOT PERMIT ALL");
    ...
    private boolean isUserAllowed(final String username, final String password, final Set<String> rolesSet) {
        for(String rolename: rolesSet) {
            if(!userService.isInRole(username, password, rolename)) {
                return false;
            }
        }
        return true;
